(defun sweep (vrp-prob)
	(let* (
		  ...
		  (vehicle  0)
		  (rem-capacity (make-list (vrp-vehicles.number vrp-prob) :initial-element vehicle-max-capacity))

		  ;Valor de Retorno
		  (clusters (make-list (vrp-vehicles.number vrp-prob) :initial-element (list))))
...
(defun calculate_angles_to_depot (depot customers)
	(let* ((angles NIL) (angle 0) (depot_location (rest depot)) (customer_id 0)
		   (customer_location NIL) (aux_point (list (+ (first depot_location) 10) (second depot_location))))
...
(defun get-angle-sweep (a b depot_location)
	...
	(let ((p-distance (distance depot_location a)) (v-size (distance depot_location b)) (v-p-distance (distance a b)))
...
(defun adjust-angle (angle depot_location point)
	(let ((point_y (second point))
		  ...
		  (my-angle angle))
	(when (< point_y (* 1 depot_y)) (setf my-angle (- 360 angle)))
	my-angle))
